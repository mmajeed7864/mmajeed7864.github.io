...
4. For a real Android development project, run `node scripts/prepare-android-project.mjs --web-bundle dist --out /absolute/new/project-directory` and follow [ANDROID_BUILD.md](ANDROID_BUILD.md). It preserves the original source/scaffolds and automates the reviewed Gradle, Kotlin, launcher, privacy, icons and local-web-asset integration. Release identity/signing remains a separate gate. For iOS, generate the project and merge the included AppDelegate/SceneDelegate/bridge-controller into its targets. The iOS bridge controller registers an app-local plugin instance because Capacitor 8 type registration is disabled after automatic registration.
