...
3. Run `npm run web:build`, then `npm run web:verify` to assemble and verify `dist/` from the checked-out web app. The builder preserves `/fitcoach-founder-test/` paths, includes declared media and font licenses, verifies media checksums, and excludes unlisted repository files. It never replaces an existing output directory; preserve/move an old `dist/` aside before rebuilding. Read [WEB_BUNDLE.md](WEB_BUNDLE.md). Do not use a hosted `server.url` for store builds.
