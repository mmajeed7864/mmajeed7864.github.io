...
test("competitive discovery, evidence, and strength modules are available offline", () => {
  ...
  assert.match(sw, /const MEDIA_CACHE = "fitcoach-exercise-images-v0600"/u);
  ...
  assert.match(sw, /if \(anatomyAsset \|\| brandAsset \|\| legalAsset\)[\s\S]*?cachedOrFetch\(event\.request, event\)/u);
});
