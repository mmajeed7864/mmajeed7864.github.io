...
test("version and cache generation agree at 0.7.6 / 0706", async () => {
  const constants = await import("../v040/core/constants.mjs");
  assert.equal(constants.BUILD, "0.7.6");
  assert.equal(constants.CACHE_GENERATION, "0706");
  const sw = readFileSync(new URL("../sw.js", import.meta.url), "utf8");
  assert.match(sw, /const CACHE = "fitcoach-symbio-v0706";/u);
  const html = readFileSync(new URL("../index.html", import.meta.url), "utf8");
  assert.match(html, /<title>FitCoach v0\.7\.6<\/title>/u);
