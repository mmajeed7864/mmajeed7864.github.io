...
test("active app has no password gate or visible founder picker", () => {
  ...
  assert.match(html, /FitCoach v0\.7\.6/u);
});
...
test("the CSP-safe boot script owns service-worker upgrades and modules refresh network-first", () => {
  ...
  assert.match(html, /<script src="\.\/v040\/boot\.js\?v=0706"><\/script>/u);
  assert.match(boot, /serviceWorker\.register\("\.\/sw\.js\?v=0706", \{ updateViaCache: "none" \}\)/u);
