...
export function createSyncCoordinator({ client, getStore, getConsentVersion, deviceId, schemaVersion, applyRemote, onState = () => {}, clock = () => new Date() }) {
  ...
  async function sync({ preference = "auto", pendingRemote = null } = {}) {
    ...
    const update = fn => { assertCurrent(); const next = store.update(fn); onState(next); return next; };
    try {
      ...
      if (!sameAccount) {
        update(draft => { draft.integrations.cloudSync = { status: "local_only", revision: 0, consentVersion: "", lastSyncedAt: null, accountScope }; });
      }
      if (!sameAccount || cloud.consentVersion !== version) {
        await client.recordSyncConsent({ policyVersion: version, decision: "accepted" });
        assertCurrent();
        update(draft => { draft.integrations.cloudSync.consentVersion = version; });
      }
      ...
      const conflict = () => {
        update(draft => { draft.integrations.cloudSync.status = "conflict"; });
        return { status: "conflict", remote };
      };
      if (preference !== "auto" && (!pendingRemote || Number(pendingRemote.revision) !== remoteRevision)) return conflict();
      const remoteSignature = remote.state ? syncStateSignature(remote.state) : null;
      const sameContent = remoteSignature === localSignature;
      if (remote.state && preference === "auto" && remoteRevision !== localRevision && dirty && !sameContent) return conflict();
      ...
      if (remote.state && (preference === "cloud" || (preference === "auto" && remoteRevision !== localRevision && !dirty && !sameContent))) {
        ...
        ensureUnchanged();
        const releaseAccessAllowed = applyRemote(remote, { lastSyncedDigest: remoteDigest, accountScope });
        const status = syncStateSignature(store.get()) === remoteSignature ? "connected" : "pending";
        update(draft => { draft.integrations.cloudSync.status = status; });
      ...
      if (sameContent) {
        ...
        update(draft => {
          draft.integrations.cloudSync = { status: "connected", revision: remoteRevision, consentVersion: version, lastSyncedAt: remote.updatedAt || clock().toISOString(), lastSyncedDigest: digest, accountScope };
        });
      ...
      const status = syncStateSignature(store.get()) === localSignature ? "connected" : "pending";
      update(draft => {
      ...
      assertCurrent();
      update(draft => { draft.integrations.cloudSync.status = "error"; });
      throw error;
    } finally {
