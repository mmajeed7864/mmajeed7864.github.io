...
export function exerciseMotionGuide(exercise, { className = "", eager = false, paused = false } = {}) {
  ...
  return `<figure class="exercise-motion ${escapeHtml(className)}" data-motion-status="loading" style="--motion-aspect:${Number(motion.width) || 720} / ${Number(motion.height) || 720}">
    <video data-media-video data-action="toggle-exercise-motion" data-motion-id="${escapeHtml(motion.id || motion.exerciseId || name)}" src="${escapeHtml(motion.path)}" ${poster?.path ? `poster="${escapeHtml(poster.path)}"` : ""} width="${motion.width || 720}" height="${motion.height || 720}" preload="auto" muted loop playsinline ${paused ? "" : "autoplay"} disablepictureinpicture aria-label="${escapeHtml(motion.alt || `${name} movement demonstration`)}"></video>
    ...
    <span class="media-play-hint">Tap the video to resume</span>
