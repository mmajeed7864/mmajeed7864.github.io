...
function renderLaunchReadiness(state) {
  ...
  return `<section class="settings-card launch-readiness-card card">
    <header class="section-heading">
      ...
      <span class="soft-badge">Honest preview</span>
    </header>
...
export function renderProfileScreen({state}) {
  ...
  return `<div class="page profile-page">
    <section class="profile-hero teal-panel"><div class="profile-avatar">${icon("profile")}</div><div><span class="eyebrow">LOCAL PROFILE</span><h1>FitCoach</h1><p>${escapeHtml(state.profile.goal)} · ${state.profile.days} days/week · ${state.profile.duration} min · ${escapeHtml(state.profile.location)}</p></div><span class="founder-chip">v${BUILD}</span></section>
    ...
    <section class="privacy-card card"><header>${icon("info")}<span><small>DATA + PRIVACY</small><h2>Know what leaves the device</h2></span></header><p>Workout logs, exercise preferences, theme, and local profile data stay in this browser in v0.4. Ordinary low-sensitivity Coach text may be sent through Symbio’s server to DeepSeek or the configured Qwen backup. Bounded coach reply text may use ElevenLabs for speech. FitCoach does not upload microphone audio. Do not enter diagnoses, medications, measurements, credentials, identifiers, or private records.</p><ul><li>Provider keys remain server-side</li><li>Plan changes always require visible approval</li><li>Exercise visuals are original local temporary diagrams</li></ul></section>
    <section class="settings-card card"><header class="section-heading"><div><span class="eyebrow">APP TOOLS</span><h2>Data and recovery</h2></div><span class="soft-badge">v${BUILD}</span></header><div class="founder-tools">${button({label:"Show tutorial",action:"open-tutorial",variant:"secondary"})}${button({label:"Export local data",action:"export-data",variant:"secondary"})}${button({label:"Refresh app assets",action:"force-refresh",variant:"secondary"})}${button({label:"Clear Coach thread",action:"clear-chat",variant:"quiet"})}${button({label:"Reset local profile",action:"reset-profile",variant:"danger"})}</div><p>Migration preserves an exact backup of the previous local payload. Sessions created only in v0.4 do not appear if you manually roll back to an older build.</p></section>
