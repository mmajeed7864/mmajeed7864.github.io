...
function cloudAccountSection(state, ui) {
  ...
  const waiting = ["pending", "error"].includes(cloud.status);
  const syncTitle = cloud.status === "pending" ? "Newest changes waiting" : cloud.status === "error" ? "Sync paused" : cloud.status === "connected" ? "Cloud sync on" : cloud.status === "conflict" ? "Sync needs your choice" : "Cloud sync off";
  const syncCopy = waiting
    ? "Your newest changes stay on this device. Sync again when the connection is ready."
