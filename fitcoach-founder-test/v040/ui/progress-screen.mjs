...
function weeklyHero(evidence, sessions, now) {
  ...
  return `<section class="progress-v6-hero"><header class="progress-v6-heading"><div><span class="progress-v6-eyebrow">YOUR PROGRESS</span><h1>${title}</h1><p>${escapeHtml(shortDate(monday(now)))} – ${escapeHtml(shortDate(now))}<span> · This week</span></p></div>${icon("progress", "progress-v6-heading-icon")}</header>
    <div class="progress-v6-week-summary"><div><span class="progress-v6-hero-number">${current.completedSessions}<small> / ${evidence.scheduleTarget}</small></span><p>workouts this week</p><span class="progress-v6-hero-note">${sessions.length} completed ${sessions.length === 1 ? "session" : "sessions"} overall</span></div><svg class="progress-v6-goal-ring" viewBox="0 0 108 108" role="img" aria-label="${current.completedSessions} of ${evidence.scheduleTarget} planned weekly workouts completed"><circle cx="54" cy="54" r="46" class="progress-v6-ring-track"/><circle cx="54" cy="54" r="46" class="progress-v6-ring-value" stroke-dasharray="${circumference}" stroke-dashoffset="${circumference * (1 - fraction)}"/><g transform="translate(42,42)">${icon("train")}</g></svg></div>
    ${weekStrip(sessions, now)}
...
function firstDayHero() {
  return `<section class="progress-v6-first-hero"><span class="progress-v6-eyebrow">YOUR PROGRESS</span><div class="progress-v6-first-art" aria-hidden="true"><span></span><span></span><span></span>${icon("progress")}</div><h1>Your baseline starts with one session.</h1><p>Start where you are. Every workout you log adds to your story.</p>${button({ label: "Choose your first workout", action: "route", value: "train", variant: "primary", iconName: "play" })}<ol class="progress-v6-first-path" aria-label="Your first three progress milestones"><li><i>1</i><span>Log a session</span></li><li><i>2</i><span>Build a rhythm</span></li><li><i>3</i><span>See your progress</span></li></ol></section>`;
}
export function renderProgressScreen({ state, now = new Date(), communityPreviews = new Map(), weeklyEvidence = null }) {
  ...
  if (!pastSessions.length) return `<div class="page progress-page progress-v6 progress-first-day">${firstDayHero()}${nutritionTrendCard(state, now)}${progressStudio(state, communityPreviews)}</div>`;
  return `<div class="page progress-page progress-v6">${weeklyHero(evidence, pastSessions, now)}${milestones(pastSessions.length)}<div class="progress-v6-grid">${volumeCard(pastSessions, unit)}${calendarCard(pastSessions, now, evidence.scheduleTarget)}</div><div class="progress-v6-grid">${bestsCard(pastSessions, unit)}${focusCard(pastSessions)}</div>${historyCard(pastSessions, unit)}${nutritionTrendCard(state, now)}${progressStudio(state, communityPreviews)}</div>`;
}
