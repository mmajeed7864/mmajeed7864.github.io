...
function progressStudio(state, communityPreviews = new Map()) {
  ...
  return `<section class="progress-studio card"><header class="section-heading"><div><span class="eyebrow">PROGRESS STUDIO</span><h2>Your photos, your timeline</h2></div><span class="soft-badge">Private by default</span></header><p>Capture a check-in, add context, and keep a private draft. A public community is intentionally locked until FitCoach has accounts, reporting, moderation, and deletion controls.</p><div class="progress-studio-actions">${button({label:"Add progress photo",action:"open-community-draft",variant:"primary",iconName:"camera"})}<span>${icon("info")}No photo is uploaded in this build.</span></div>${drafts.length ? `<div class="progress-draft-grid">${drafts.map(item => {const preview=communityPreviews.get(item.id);return `<article>${preview ? `<img src="${escapeHtml(preview)}" alt="Local progress draft preview">` : `<div class="progress-photo-placeholder">${icon("camera")}<span>Preview ended after reload</span></div>`}<div><small>${escapeHtml(formatDate(item.createdAt))} · ${escapeHtml(item.visibility === "private" ? "Only me" : "Unpublished draft")}</small><b>${escapeHtml(item.caption || "Progress check-in")}</b><button data-action="delete-community-draft" data-value="${escapeHtml(item.id)}">Delete local draft</button></div></article>`;}).join("")}</div>` : `<div class="progress-studio-empty">${icon("camera")}<span><b>No progress photos yet</b><small>Your first private check-in will appear here.</small></span></div>`}</section>`;
}

export function renderProgressScreen({state,now=new Date(),communityPreviews=new Map()}) {
  ...
  if (!pastSessions.length) {
    return `<div class="page progress-page progress-first-day">
      <section class="progress-empty-hero teal-panel"><div class="first-proof-mark" aria-hidden="true">${icon("progress")}</div><span class="eyebrow">YOUR PROGRESS STARTS HERE</span><h1>No fake history. Your first workout creates the baseline.</h1><p>FitCoach will turn completed sets into strength trends, weekly consistency, muscle balance, and personal bests. Until you log something, the record stays honestly empty.</p>${button({label:"Choose your first workout",action:"route",value:"train",variant:"primary",iconName:"play"})}</section>
      <section class="first-progress-guide card"><header class="section-heading"><div><span class="eyebrow">WHAT APPEARS NEXT</span><h2>Useful proof—not vanity numbers</h2></div></header><div><article><span>01</span><b>Finish a workout<small>Only completed sets enter your history.</small></b></article><article><span>02</span><b>See your baseline<small>Loads, reps, and volume become the starting point.</small></b></article><article><span>03</span><b>Get the next target<small>Your trainer can explain a progression without silently changing the plan.</small></b></article></div></section>
