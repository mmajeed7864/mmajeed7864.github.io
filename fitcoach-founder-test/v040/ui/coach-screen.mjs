...
export function renderCoachScreen({ state, decision, ui, coachConnection }) {
  ...
  return `<div class="page coach-page coach-v060" data-tone="${escapeHtml(state.profile.tone.toLowerCase())}">
    <header class="mentor-heading"><span class="mentor-eyebrow">YOUR AI TRAINER</span><h1>A coach in your corner.</h1><div class="mentor-identity"><span class="mentor-avatar" aria-hidden="true">${icon("spark")}</span><div><b>${escapeHtml(trainerName)}</b><small>${escapeHtml(state.profile.tone)} · ${escapeHtml(TONE_SHORT[state.profile.tone] || "Your training partner")}</small></div></div></header>
    <section class="mentor-voice-card" aria-labelledby="mentor-voice-title">
      <div class="mentor-voice-intro"><div><span class="mentor-eyebrow">VOICE ROOM</span><h2 id="mentor-voice-title">Talk naturally. Hear the answer.</h2></div><div class="mentor-voice-art" aria-hidden="true"><svg viewBox="0 0 150 150" fill="none"><circle cx="75" cy="75" r="58" fill="#274c92" fill-opacity=".24"/><ellipse cx="75" cy="75" rx="31" ry="54" stroke="#85b4ff" stroke-width="2.4" transform="rotate(-35 75 75)"/><ellipse cx="75" cy="75" rx="31" ry="54" stroke="#b2a7fa" stroke-width="2.4" transform="rotate(35 75 75)"/><ellipse cx="75" cy="75" rx="31" ry="54" stroke="#739af4" stroke-width="1.4" transform="rotate(90 75 75)"/><circle cx="75" cy="75" r="13" fill="#dce9ff"/><circle cx="75" cy="75" r="6" fill="#fff"/></svg></div></div>
      ${button({label:"Start Voice Room",action:"open-voice-room",variant:"voice",iconName:"mic"})}
      <div class="mentor-voice-foot"><span>Speak. Listen. Keep moving.</span><button type="button" class="mentor-connection" data-action="connection-info" data-connection="${escapeHtml(connection.state)}" aria-label="Coach connection: ${escapeHtml(connection.label)}. Show connection details"><i aria-hidden="true"></i><span role="status">${ui.speakingMessageId ? "Speaking now" : escapeHtml(connection.label)}</span>${icon("info")}</button></div>
    </section>
    ...
    <section class="chat-surface card mentor-conversation" aria-label="Your conversation with the trainer">
      <header><span><b>Your conversation</b><small>Always here in text, too</small></span><button class="icon-only" data-action="clear-chat" aria-label="Clear chat">${icon("close")}</button></header>
      <div id="chat-log" class="chat-log" role="log" aria-live="polite">${state.chat.length ? state.chat.map(message => renderMessage(message,ui.speakingMessageId,trainerName)).join("") : `<div class="mentor-welcome"><span aria-hidden="true">${icon("spark")}</span><div><h2>What would make today easier?</h2><p>A shorter session, a technique check, or a fresh start.</p></div></div>`}${ui.pendingMessage ? `<article class="chat-message user pending"><div>${escapeHtml(ui.pendingMessage)}</div><footer><span>Sending your message…</span></footer></article>` : ""}${ui.chatNotice ? `<article class="chat-notice ${escapeHtml(ui.chatNotice.kind)}"><b>${escapeHtml(ui.chatNotice.title)}</b><p>${escapeHtml(ui.chatNotice.message)}</p>${ui.chatNotice.retryable ? button({label:"Restore draft",action:"restore-chat-draft",variant:"secondary"}) : ""}</article>` : ""}${ui.chatBusy ? `<div class="typing-indicator" aria-label="Trainer is thinking"><i></i><i></i><i></i></div>` : ""}</div>
      <div class="coach-composer"><button class="icon-only mic-button" data-action="open-voice-room" aria-label="Open Voice Room">${icon("mic")}</button><label><span class="sr-only">Message your trainer</span><textarea id="coach-input" rows="1" maxlength="2000" placeholder="Tell ${escapeHtml(trainerName)} what’s on your mind" ${ui.chatBusy ? "disabled" : ""}>${escapeHtml(ui.chatDraft || "")}</textarea></label><button class="icon-only send-button" data-action="send-chat" aria-label="Send message" ${ui.chatBusy ? "disabled" : ""}>${icon("send")}</button></div>
    ...
    <details class="coach-disclosure trainer-memory card" data-disclosure="coach-memory" ${ui.disclosures?.["coach-memory"] ? "open" : ""}><summary><span class="section-heading"><span><span class="eyebrow">YOUR TRAINING CONTEXT</span><h2>What I work with</h2></span></span><span class="disclosure-current">You’re in control</span><span class="disclosure-chevron" aria-hidden="true">${icon("chevron")}</span></summary><div class="disclosure-body"><div>${trainerFacts.map(([label,value]) => `<span><small>${escapeHtml(label)}</small><b>${escapeHtml(String(value))}</b></span>`).join("")}</div><section class="mentor-decision"><span class="eyebrow">WHY THIS COACHING</span><h3>${escapeHtml(decision?.title || "Built from your current plan")}</h3><button class="text-button" data-action="explain-decision">See the evidence ${icon("chevron")}</button></section><footer>${icon("info")}<p>Your full profile stays on this device. The live trainer receives only the limited context needed to answer this turn. Links and plan options are selected inside FitCoach.</p></footer></div></details>
  </div>`;
}
...
export function renderVoiceRoom(voiceState, state, { docked = false } = {}) {
  ...
  if (docked) {
    return `<aside class="voice-dock" role="region" aria-label="Active trainer voice session" data-phase="${escapeHtml(voiceState.phase)}">
      <button class="voice-dock-main" data-action="voice-expand" aria-label="Expand Voice Room"><span class="voice-dock-orb" aria-hidden="true"><i></i><b></b></span><span class="voice-dock-copy"><small>${escapeHtml(title)}</small><b>${escapeHtml(trainerName)} · ${escapeHtml(state.profile.tone)}</b><em>${escapeHtml(trainerText || transcript || "Voice session stays active while you use FitCoach.")}</em></span></button>
