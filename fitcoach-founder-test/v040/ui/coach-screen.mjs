...
export function renderCoachScreen({ state, decision, ui, coachConnection }) {
  ...
  return `<div class="page coach-page" data-tone="${escapeHtml(state.profile.tone.toLowerCase())}">
    <section class="coach-hero teal-panel">
      ...
      <div class="voice-room-launch"><div class="voice-launch-orb" aria-hidden="true"><i></i><span></span></div><div><span class="eyebrow">LIVE VOICE</span><h2>Talk naturally. Hear the answer.</h2><p>Persistent captions, automatic spoken replies, and one-tap interruption.</p></div>${button({label:"Start Voice Room",action:"open-voice-room",variant:"voice",iconName:"mic"})}</div>
      <div class="provider-boundary compact"><span>DeepSeek</span><span>Qwen backup</span><span>${escapeHtml(speechStatus)}</span></div>
...
export function renderVoiceRoom(voiceState, state) {
  ...
  return `<section id="voice-room" class="voice-room" role="dialog" aria-modal="true" aria-labelledby="voice-room-title" data-phase="${escapeHtml(voiceState.phase)}" data-tone="${escapeHtml(state.profile.tone.toLowerCase())}">
    ...
    ${voiceState.phase === "consent" ? `<section class="voice-consent"><div class="voice-consent-points"><span>${icon("mic")}<b>Mic stays on device</b></span><span>${icon("volume")}<b>Replies speak automatically</b></span><span>${icon("close")}<b>End anytime</b></span></div>${button({label:"Start talking",action:"voice-consent",variant:"primary",iconName:"mic"})}<details><summary>Voice privacy</summary><p>Your device creates the transcript. FitCoach sends only that bounded text to Symbio for DeepSeek or Qwen, then may send the safe reply text to ElevenLabs. FitCoach does not upload microphone audio. Avoid private medical details, identifiers, and credentials.</p></details></section>` : ""}
    ...
    <small class="voice-privacy">Automatic ElevenLabs reply · device fallback · captions stay in this chat</small>
  </section>`;
}
