...
// A phase indicator, not a microphone-level visualization. The controller owns
// listening and playback; only its real phases animate this decorative mark.
function voiceSignal() {
  return `<svg class="voice-v7-signal" viewBox="0 0 240 240" fill="none" aria-hidden="true" focusable="false"><circle class="voice-v7-track" cx="120" cy="120" r="111"/><circle class="voice-v7-ring voice-v7-ring-outer" cx="120" cy="120" r="111" stroke-dasharray="112 20 42 20 112 20 42 329"/><circle class="voice-v7-ring voice-v7-ring-inner" cx="120" cy="120" r="94" stroke-dasharray="220 36 98 237"/><circle class="voice-v7-core" cx="120" cy="120" r="76"/><path class="voice-v7-mic" d="M109 103a11 11 0 0 1 22 0v23a11 11 0 0 1-22 0zm-8 22a19 19 0 0 0 38 0m-19 19v13m-10 0h20"/></svg>`;
}
...
export function renderCoachScreen({ state, decision, ui, coachConnection }) {
  ...
  const trainerName = voiceName.split(" · ")[0];
  const pending = state.pendingPlanProposal;
  const connection = coachConnection || { label: "Ready when you are", state: "unverified" };
  const workout = state.activeWorkout;
  const currentExercise = workout?.exercises?.[Math.max(0, Math.min(workout.currentExerciseIndex || 0, workout.exercises.length - 1))];
  ...
  return `<div class="page coach-page coach-v070" data-tone="${escapeHtml(state.profile.tone.toLowerCase())}">
    <header class="coach-v7-masthead"><div class="coach-v7-kicker"><span>YOUR PERSONAL TRAINING SPACE</span><span aria-hidden="true">${icon("spark")}</span></div><h1>COACH<span aria-hidden="true">.</span></h1><div class="coach-v7-identity"><span class="coach-v7-monogram" aria-hidden="true">${escapeHtml(trainerName.slice(0,1))}</span><div><b>${escapeHtml(trainerName)}</b><span>${escapeHtml(state.profile.tone)} · ${escapeHtml(TONE_SHORT[state.profile.tone] || "Your training partner")}</span></div><span class="coach-v7-identity-note">IN YOUR<br>CORNER</span></div></header>
    <section class="coach-v7-stage" aria-labelledby="coach-v7-stage-title" data-phase="${ui.speakingMessageId ? "speaking" : "idle"}">
      <div class="coach-v7-stage-copy"><span class="coach-v7-label">VOICE ROOM</span><h2 id="coach-v7-stage-title">Real talk.<br>Next move.</h2><p>Talk naturally. Hear the answer.</p></div>
      <button type="button" class="coach-v7-start" data-action="open-voice-room" aria-label="Start Voice Room">${voiceSignal()}<span>Start Voice Room ${icon("chevron")}</span></button>
      <footer class="coach-v7-stage-footer"><span>${ui.speakingMessageId ? "Reply playing" : "You choose when to begin"}</span><button type="button" class="mentor-connection" data-action="connection-info" data-connection="${escapeHtml(connection.state)}" aria-label="Coach connection: ${escapeHtml(connection.label)}. Show connection details"><i aria-hidden="true"></i><span role="status">${escapeHtml(connection.label)}</span>${icon("info")}</button></footer>
    </section>

    ${currentExercise ? `<section class="mentor-workout-context" aria-label="Current workout context"><div class="mentor-workout-mark" aria-hidden="true">${icon("train")}</div><div class="mentor-workout-copy"><small>WORKOUT IN PROGRESS</small><b>${escapeHtml(currentExercise.snapshot?.name || workout.planLabel || "Your current exercise")}</b><div><button data-action="ask-about-exercise" data-value="${escapeHtml(currentExercise.exerciseId)}">Ask about this move</button><button data-action="resume-workout">Resume workout ${icon("chevron")}</button></div></div></section>` : ""}

    ${pending ? `<section class="plan-proposal coach-proposal"><span class="eyebrow">PLAN CHANGE PREVIEW</span><h2>${escapeHtml(pending.candidate.label)} · ${pending.candidate.minutes} min</h2><p>${escapeHtml(pending.reason)}</p><ul>${pending.changes.map(change => `<li>${escapeHtml(change)}</li>`).join("")}</ul><footer>${button({label:"Keep current plan",action:"reject-proposal",value:pending.id,variant:"quiet"})}${button({label:"Approve change",action:"approve-proposal",value:pending.id,variant:"primary"})}</footer></section>` : ""}

    <section class="chat-surface coach-v7-conversation" aria-label="Your conversation with the trainer">
      <header><span><small>THE CONVERSATION</small><h2>Make your next move.</h2></span><button type="button" class="icon-only" data-action="clear-chat" aria-label="Clear chat">${icon("close")}</button></header>
      <div id="chat-log" class="chat-log" role="log" aria-live="polite" aria-relevant="additions text">${state.chat.length ? state.chat.map(message => renderMessage(message,ui.speakingMessageId,trainerName)).join("") : `<div class="coach-v7-welcome"><span aria-hidden="true">“</span><div><h3>What would make<br>today easier?</h3><p>Big question. Small adjustment.<br>Start wherever you are.</p></div></div>`}${ui.pendingMessage ? `<article class="chat-message user pending"><div>${escapeHtml(ui.pendingMessage)}</div><footer><span>Sending your message…</span></footer></article>` : ""}${ui.chatNotice ? `<article class="chat-notice ${escapeHtml(ui.chatNotice.kind)}"><b>${escapeHtml(ui.chatNotice.title)}</b><p>${escapeHtml(ui.chatNotice.message)}</p>${ui.chatNotice.retryable ? button({label:"Restore draft",action:"restore-chat-draft",variant:"secondary"}) : ""}</article>` : ""}${ui.chatBusy ? `<div class="typing-indicator" role="status" aria-label="Trainer is thinking"><i></i><i></i><i></i></div>` : ""}</div>
      <div class="coach-composer"><button class="icon-only mic-button" data-action="open-voice-room" aria-label="Open Voice Room">${icon("mic")}</button><label><span class="sr-only">Message your trainer</span><textarea id="coach-input" rows="1" maxlength="2000" placeholder="Message ${escapeHtml(trainerName)}…" ${ui.chatBusy ? "disabled" : ""}>${escapeHtml(ui.chatDraft || "")}</textarea></label><button class="icon-only send-button" data-action="send-chat" aria-label="Send message" ${ui.chatBusy ? "disabled" : ""}>${icon("send")}</button></div>
      ...
      <details class="mentor-privacy"><summary>${icon("lock")}Privacy & coaching limits ${icon("chevron")}</summary><footer class="chat-privacy">Training guidance, not medical care. You approve every plan change. Your workout data stays on this device. Voice uses your device transcript and sends only the trainer reply text for speech—never microphone audio. Your browser or operating system may process speech to create that transcript. Avoid entering private medical or account information.</footer></details>
    </section>

    <div class="coach-v7-control-deck"><details class="coach-disclosure coach-personality" data-disclosure="coach-style" ${ui.disclosures?.["coach-style"] ? "open" : ""}><summary><span class="section-heading"><span><span class="eyebrow">MAKE IT YOURS</span><h2>Coach preferences</h2></span></span><span class="disclosure-current">${escapeHtml(state.profile.tone)} · ${escapeHtml(trainerName)}</span><span class="disclosure-chevron" aria-hidden="true">${icon("chevron")}</span></summary><div class="disclosure-body"><div class="coach-setting-group"><span>Trainer tone</span><div role="radiogroup" aria-label="Trainer tone">${TRAINER_TONES.map(value => `<button role="radio" aria-checked="${state.profile.tone === value}" class="tone-choice ${state.profile.tone === value ? "active" : ""}" data-action="set-tone" data-value="${value}"><b>${value}</b><small>${escapeHtml(TONE_SHORT[value])}</small></button>`).join("")}</div></div><div class="coach-preference-block"><span>Answer length</span><div class="preference-pill-grid" role="radiogroup" aria-label="Answer length">${Object.entries(MODEL_MODES).map(([key,value]) => `<button role="radio" aria-checked="${state.settings.coachMode === key}" class="preference-pill ${state.settings.coachMode === key ? "active" : ""}" data-action="set-answer-depth" data-value="${key}"><b>${escapeHtml(value.label)}</b><small>${escapeHtml(value.detail)}</small></button>`).join("")}</div></div><div class="coach-preference-block"><span>Trainer voice</span><div class="voice-choice-grid" role="radiogroup" aria-label="Trainer voice">${VOICE_PERSONAS.map(value => `<button role="radio" aria-checked="${state.settings.voicePersona === value}" class="voice-choice ${state.settings.voicePersona === value ? "active" : ""}" data-action="set-voice-persona" data-value="${value}"><i aria-hidden="true"></i><b>${escapeHtml(VOICE_LABELS[value].split(" · ")[0])}</b><small>${escapeHtml(VOICE_LABELS[value].split(" · ")[1] || "trainer")}</small></button>`).join("")}</div></div><p class="style-boundary">Tone, answer length, and voice change presentation only. Safety rules and plan decisions stay the same.</p></div></details>

    <details class="coach-disclosure trainer-memory" data-disclosure="coach-memory" ${ui.disclosures?.["coach-memory"] ? "open" : ""}><summary><span class="section-heading"><span><span class="eyebrow">YOUR TRAINING CONTEXT</span><h2>What I work with</h2></span></span><span class="disclosure-current">You’re in control</span><span class="disclosure-chevron" aria-hidden="true">${icon("chevron")}</span></summary><div class="disclosure-body"><div>${trainerFacts.map(([label,value]) => `<span><small>${escapeHtml(label)}</small><b>${escapeHtml(String(value))}</b></span>`).join("")}</div><section class="mentor-decision"><span class="eyebrow">WHY THIS COACHING</span><h3>${escapeHtml(decision?.title || "Built from your current plan")}</h3><button class="text-button" data-action="explain-decision">See the evidence ${icon("chevron")}</button></section><footer>${icon("info")}<p>Your full profile stays on this device. The live trainer receives only the limited context needed to answer this turn. Links and plan options are selected inside FitCoach.</p></footer></div></details></div>
  </div>`;
}
...
const PHASE_COPY = Object.freeze({
  ...
  speaking: ["Speaking automatically", "Tap the microphone to interrupt."],
...
export function renderVoiceRoom(voiceState, state, { docked = false } = {}) {
  ...
  if (docked) {
    return `<aside class="voice-dock" role="region" aria-label="Active trainer voice session" data-phase="${escapeHtml(voiceState.phase)}">
      <button class="voice-dock-main" data-action="voice-expand" aria-label="Expand Voice Room"><span class="voice-dock-orb" aria-hidden="true">${voiceSignal()}</span><span class="voice-dock-copy"><small>${escapeHtml(title)}</small><b>${escapeHtml(trainerName)} · ${escapeHtml(state.profile.tone)}</b><em>${escapeHtml(trainerText || transcript || "Voice session stays active while you use FitCoach.")}</em></span></button>
  ...
  return `<section id="voice-room" class="voice-room" role="dialog" aria-modal="true" aria-labelledby="voice-room-title" data-phase="${escapeHtml(voiceState.phase)}" data-tone="${escapeHtml(state.profile.tone.toLowerCase())}">
    ...
    <main><div class="voice-v7-session-heading"><span class="coach-v7-label">YOUR VOICE ROOM</span><p class="voice-state-label">${escapeHtml(title)}</p><h2 id="voice-room-title">${escapeHtml(copy)}</h2></div><button type="button" class="voice-room-orb" data-action="voice-interrupt" aria-label="${voiceState.phase === "speaking" ? "Interrupt trainer" : "Voice status"}">${voiceSignal()}</button><div class="voice-captions" aria-live="polite">${transcript ? `<p class="voice-user-caption"><span>You</span>${escapeHtml(transcript)}</p>` : ""}${trainerText ? `<p class="voice-trainer-caption"><span>${escapeHtml((VOICE_LABELS[state.settings.voicePersona] || VOICE_LABELS.nova).split(" · ")[0])}</span>${escapeHtml(trainerText)}</p>` : `<p class="voice-status-caption">${escapeHtml(voiceState.caption?.text || "Captions will appear here.")}</p>`}</div></main>
