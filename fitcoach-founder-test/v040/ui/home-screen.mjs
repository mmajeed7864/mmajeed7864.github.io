...
export function renderTodayScreen({ state, plan, decision, exerciseById, now = new Date() }) {
  ...
  const heroImage = '<img class="home-cover-image" src="/fitcoach-founder-test/v040/assets/brand/training-day-v060.webp" srcset="/fitcoach-founder-test/v040/assets/brand/training-day-v060-small.webp 640w, /fitcoach-founder-test/v040/assets/brand/training-day-v060.webp 960w" sizes="(min-width:700px) 550px, 100vw" width="960" height="640" fetchpriority="high" alt="">';
  ...
  return `<div class="page today-page daily-board-page home-v6">
    <header class="home-greeting"><div><span class="eyebrow">${escapeHtml(now.toLocaleDateString([], { weekday: "long", month: "short", day: "numeric" }))}</span><h1>${greeting}</h1><p>A little stronger. A little more you.</p></div><button class="home-week-link" data-action="route" data-value="progress" aria-label="View this week’s progress">${icon("progress")}<span>${weekSummary}</span></button></header>
    <div class="home-layout">
      <section class="home-session" aria-labelledby="home-session-title"><div class="home-session-media">${heroImage}</div><div class="home-session-shade"></div><div class="home-session-top"><span class="home-status"><i></i>${board.activeWorkout ? "SESSION IN PROGRESS" : board.trainedToday ? "TODAY, COMPLETED" : "MADE FOR YOUR TODAY"}</span><button data-action="route" data-value="train" class="home-session-menu" aria-label="Preview and adjust your workout">${icon("chevron")}</button></div><div class="home-session-copy"><span class="home-session-label">${escapeHtml(plan.label)} <i>•</i> ${escapeHtml(plan.location)}</span><h2 id="home-session-title">${board.activeWorkout ? "Your session is waiting." : board.trainedToday ? "You showed up." : workoutTitle(plan)}</h2><div class="home-session-meta"><span>${icon("clock")}${sessionTiming}</span><span>${icon("train")}${plan.exercises.length} exercises</span></div>${button({ label: board.activeWorkout ? "Resume workout" : board.trainedToday ? "View your session" : "Let’s train", action: primary.action, value: primary.value, variant: "primary", iconName: board.trainedToday ? "check" : "play" })}</div></section>
