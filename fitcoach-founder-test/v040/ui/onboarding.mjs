...
function boundaryStep(draft) {
  return `<div class="onboarding-step trainer-interview single-question boundary-step"><span class="eyebrow">CLEAR BOUNDARIES</span><h1>Useful, honest, and still in your control.</h1><p>One last check before we train.</p>${trainerBubble("You stay in control of every plan change.")}<div class="boundary-list premium-boundaries"><article>${icon("check")}<span><b>Your workout history stays on this device</b><small>Plans, sets, preferences, and progress remain local.</small></span></article><article>${icon("check")}<span><b>You approve every plan change</b><small>The trainer can explain or prepare an option—never activate it alone.</small></span></article><article>${icon("close")}<span><b>FitCoach is not medical care</b><small>No diagnosis, rehabilitation, medication, emergency care, or camera-based form assessment.</small></span></article><article>${icon("info")}<span><b>Check with a doctor first</b><small>Check with a doctor or qualified healthcare provider before starting or materially changing a training or nutrition plan.</small></span></article></div><label class="consent-row premium-consent"><input type="checkbox" data-action="onboarding-consent" ${draft.consent?"checked":""}><span><b>I understand FitCoach is a fitness tool, not medical care.</b><small>I will not enter medical records, account secrets, or other highly sensitive information.</small></span></label></div>`;
}
