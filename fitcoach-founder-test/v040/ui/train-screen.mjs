...
function exerciseLibrary({ state, exercises, filters }) {
  ...
  const equipment = [...new Set(exercises.flatMap(item => item.equipment))].slice(0, 8);
...
function exerciseDetail({ state, exercise, motionPaused = false, replacing = false }) {
  ...
  return `<article class="exercise-detail">
    ...
    <section class="exercise-detail-visual card static-guide"><div class="guide-stage">${exercisePoster(exercise,{className:"large concept-art",eager:true})}</div><div class="media-facts"><span>${escapeHtml(media?.view || "side/front")} guide</span><span>Original local art</span><span>Not form analysis</span></div><span class="media-license">${escapeHtml(media?.alt || "Original local guide")}</span></section>
