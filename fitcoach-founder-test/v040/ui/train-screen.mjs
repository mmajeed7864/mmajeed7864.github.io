...
function adjustmentTile({ label, detail, action, field, value, active = false }) {
  return `<button class="choice-tile ${active ? "active" : ""}" data-action="${escapeHtml(action)}" data-field="${escapeHtml(field)}" data-value="${escapeHtml(value)}" aria-pressed="${active}"><b>${escapeHtml(label)}</b><small>${escapeHtml(detail)}</small>${active ? icon("check") : ""}</button>`;
}
...
function workoutPlan({ state, plan, exerciseById }) {
  ...
  const previewExercises = plan.exercises.slice(0, 3).map(item => exerciseById(item.exerciseId)).filter(Boolean);
  const focusTitle = workoutFocusTitle(muscles);
  return `<div class="train-workout-view">
    <section class="workout-hero teal-panel">
      <div class="workout-hero-visual" aria-hidden="true"><div class="plan-preview-stack">${previewExercises.map((exercise, index) => exercisePoster(exercise, { className: `plan-preview-poster preview-${index + 1}`, eager: index === 0, label: false })).join("")}</div><span><b>${plan.exercises.length}</b><small>moves ready</small></span></div>
      <div class="workout-hero-copy"><span class="eyebrow">${escapeHtml(plan.label.toUpperCase())} · ${escapeHtml(plan.detail.toUpperCase())}</span><h1>${escapeHtml(focusTitle)}</h1><p>${escapeHtml(titleCase(plan.goal))} · ${escapeHtml(titleCase(plan.location))}</p><div class="workout-metrics"><span>${icon("clock")}<small>Duration</small><b>${plan.minutes} min</b></span><span>${icon("train")}<small>Session</small><b>${plan.exercises.length} moves</b></span></div>${button({ label: state.activeWorkout ? "Resume workout" : "Start workout", action: state.activeWorkout ? "resume-workout" : "start-workout", value: plan.id, variant: "primary", iconName: "play" })}</div>
    </section>

    <section class="plan-adjust card"><header class="section-heading"><div class="adjust-heading-mark">${icon("spark")}</div><div><span class="eyebrow">TUNE TODAY</span><h2>Fit the workout to real life</h2></div><span class="soft-badge">You confirm</span></header><div class="control-block"><span>How much time do you have?</span><div class="choice-row choice-tile-row minutes">${SESSION_MINUTES.map(value => adjustmentTile({ label: String(value), detail: "minutes", action: "propose-plan", field: "minutes", value: String(value), active: plan.minutes === value })).join("")}</div></div><div class="control-grid"><div class="control-block"><span>Where are you training?</span><div class="choice-row choice-tile-row compact">${[
      ["gym", "Gym", "Full equipment"],
      ["home", "Home", "Keep it simple"],
      ["travel", "Travel", "Minimal setup"],
    ].map(([value, label, detail]) => adjustmentTile({ label, detail, action: "propose-plan", field: "location", value, active: plan.location === value })).join("")}</div></div><div class="control-block"><span>How hard should today feel?</span><div class="choice-row choice-tile-row compact">${[
      ["light", "Light", "Leave fresh"],
      ["standard", "Standard", "Planned effort"],
...
function exerciseLibrary({ state, exercises, filters }) {
  ...
  return `<div class="exercise-library-view">
    ...
    <section class="library-tools card"><div class="library-search-row"><label class="search-field">${icon("search")}<input id="exercise-search" type="search" maxlength="80" placeholder="Search exercises" value="${escapeHtml(filters.query || "")}"><button data-action="clear-exercise-search" aria-label="Clear search">${icon("close")}</button></label><label class="favorite-filter ${filters.favorites ? "active" : ""}"><input class="sr-only" type="checkbox" data-action="filter-favorites" ${filters.favorites ? "checked" : ""}><span>${icon("heart")}</span><small>Favorites</small></label></div><div class="filter-group"><span>Muscle</span><div class="filter-scroll" aria-label="Muscle filters"><button class="filter-chip ${!filters.muscle ? "active" : ""}" data-action="filter-exercises" data-field="muscle" data-value="">All</button>${muscles.map(value => `<button class="filter-chip ${filters.muscle === value ? "active" : ""}" data-action="filter-exercises" data-field="muscle" data-value="${escapeHtml(value)}">${escapeHtml(value)}</button>`).join("")}</div></div><div class="filter-group"><span>Equipment</span><div class="filter-scroll" aria-label="Equipment filters"><button class="filter-chip ${!filters.equipment ? "active" : ""}" data-action="filter-exercises" data-field="equipment" data-value="">Any</button>${equipment.map(value => `<button class="filter-chip ${filters.equipment === value ? "active" : ""}" data-action="filter-exercises" data-field="equipment" data-value="${escapeHtml(value)}">${escapeHtml(value)}</button>`).join("")}</div></div></section>
