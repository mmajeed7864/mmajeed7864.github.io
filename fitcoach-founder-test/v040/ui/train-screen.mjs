...
import { restSecondsRemaining, restTimerKey, workoutPauseKey } from "../domain/workouts.mjs";
...
function trainingExerciseRow(item, index, exercise, units) {
  ...
  return `<article class="training-movement" data-plan-exercise="${index}">
    <span class="training-movement-number" aria-hidden="true">${String(index + 1).padStart(2, "0")}</span>
    <button class="training-movement-art" data-action="open-exercise" data-value="${escapeHtml(item.exerciseId)}" aria-label="View ${escapeHtml(name)} technique">${exercisePoster(exercise || item, { className: "training-row-poster", label: false })}</button>
    <button class="training-movement-copy" data-action="open-exercise" data-value="${escapeHtml(item.exerciseId)}"><small>${escapeHtml(muscles.slice(0, 2).join(" · ") || "Full body")}</small><b>${escapeHtml(name)}</b><span><strong>${target.sets || 0} × ${target.reps || 0}</strong>${target.suggestedWeight ? ` · ${target.suggestedWeight} ${escapeHtml(units)}` : ""} · ${target.restSeconds || 90}s rest</span>${exerciseMotionMedia(exercise) ? `<em>${icon("play")} Watch technique</em>` : ""}</button>
...
function workoutPlan({ state, plan, exerciseById }) {
  ...
  const scheduledIndex = String(plan.id || "").match(/^schedule-([1-7])$/)?.[1];
  const sessionCode = ({ A: "01", B: "02", MIN: "MIN" })[plan.id] || (scheduledIndex ? scheduledIndex.padStart(2,"0") : "—");
  return `<div class="train-workout-view training-workout">
    <header class="training-session-title"><div class="training-edition"><span>${escapeHtml(plan.label)} / ${escapeHtml(titleCase(plan.location))}</span><button class="training-inline-button" data-action="train-segment" data-value="schedule">The week ${icon("chevron")}</button></div><h1><span>SESSION</span><b>${escapeHtml(sessionCode)}</b></h1><div class="training-session-subtitle"><h2>${escapeHtml(focusTitle)}</h2><span>${escapeHtml(titleCase(plan.goal))}</span></div></header>
    ...
    <section class="training-session-list"><header class="training-section-heading"><div><span class="eyebrow">YOUR TRAINING LOGBOOK</span><h2>The lineup.</h2></div><button class="training-inline-button" data-action="add-exercise">${icon("plus")} Add move</button></header><div class="training-warmup"><span>${icon("flame")}</span><div><b>First, find your rhythm.</b><small>${plan.warmupMinutes} min warm-up · ${plan.cooldownMinutes} min cooldown</small></div></div><div class="training-movement-list">${plan.exercises.map((item,index) => trainingExerciseRow(item,index,exerciseById(item.exerciseId),state.settings.units)).join("")}</div><footer class="training-list-footer">${button({ label: "Save routine", action: "save-routine", variant: "quiet", iconName: "plus" })}<button class="training-inline-button" data-action="why-workout">Why this workout ${icon("chevron")}</button></footer></section>
    <button class="training-coach-link" data-action="route" data-value="coach"><span>${icon("spark")}</span><span><small>IN YOUR CORNER</small><b>LET’S TALK TRAINING.</b></span>${icon("chevron")}</button>
  </div>`;
}
...
function exerciseLibrary({ state, exercises, exerciseLibrary: library, filters }) {
  ...
  return `<div class="exercise-library-view training-library">
    <header class="training-library-masthead"><span class="eyebrow">THE MOVEMENT ARCHIVE</span><h1>BUILT<br>TO MOVE<span aria-hidden="true">↗</span></h1><div><strong>${catalogue.length}</strong><p>Ways to get stronger.<br>Find the movement that fits.</p></div><p class="sr-only">${catalogue.length} exercises. Every move, made clear.</p></header>
...
function scheduleCard(slot) {
  return `<article class="schedule-card">
    <div class="training-schedule-day"><span class="schedule-day">${escapeHtml(slot.shortDayLabel)}</span><small>${slot.minutes} MIN</small></div>
...
function scheduleView({ state, workoutSchedule = [], progressionRows = [] }) {
  ...
  return `<div class="schedule-view training-schedule">
    <section class="training-schedule-heading">
      <span class="eyebrow">MAKE ROOM FOR YOURSELF</span>
      <h1>THE<br>TRAINING<br>WEEK<span aria-hidden="true">.</span></h1>
...
function exerciseDetail({ state, exercise, motionPaused = false, replacing = false }) {
  ...
  return `<article class="exercise-detail training-detail">
    ...
    <header class="training-detail-heading"><span class="eyebrow">MOVEMENT STUDY / ${escapeHtml(exercise.movementPattern.replaceAll("-", " ").toUpperCase())}</span><h1>${escapeHtml(exercise.name)}</h1><p>${escapeHtml(titleCase(exercise.primaryMuscles.join(" · ")))}</p></header>
    ...
    <section class="training-key-cue"><span aria-hidden="true">01</span><div><small>YOUR ONE THING</small><p>${escapeHtml(exercise.keyCues[0] || exercise.executionSteps[0] || "Move with control.")}</p></div></section>
...
function activeWorkout({ state, exerciseById, now, ui }) {
  ...
  const identityAttrs = `data-workout-id="${escapeHtml(workout.id)}" data-exercise-id="${escapeHtml(current.exerciseId)}" data-set-id="${escapeHtml(current.sets[0]?.id || "")}"`;
  const restAttrs = `data-workout-id="${escapeHtml(workout.id)}" data-rest-key="${escapeHtml(restTimerKey(workout))}"`;
  return `<div class="active-workout-view training-active">
    <header class="active-workout-top concept-nav"><button class="icon-only" data-action="minimize-workout" aria-label="Minimize workout">←</button><span><b>${escapeHtml(workout.planLabel)}</b><small>${paused ? "SESSION PAUSED" : "SESSION IN PROGRESS"}</small></span><button class="text-button danger" data-action="exit-workout" data-workout-id="${escapeHtml(workout.id)}">End</button></header>
    <header class="training-live-heading"><span class="eyebrow">MOVEMENT ${String(currentIndex+1).padStart(2,"0")} / ${String(workout.exercises.length).padStart(2,"0")}</span><h1>${escapeHtml(current.snapshot.name)}</h1></header>
    ...
    <section class="guide-action-list card"><button data-action="view-current-instructions" ${identityAttrs}>${icon("equipment")}<span>Setup</span>${icon("chevron")}</button><button data-action="view-current-instructions" ${identityAttrs}>${icon("play")}<span>How to move</span>${icon("chevron")}</button><button data-action="view-current-instructions" ${identityAttrs}>${icon("info")}<span>Common mistakes</span>${icon("chevron")}</button></section>
    <section class="workout-progress-line training-set-track"><div class="training-set-markers" aria-hidden="true">${allSets.map(set => `<span class="${set.done ? "is-logged" : ""}"></span>`).join("")}</div><p><b>${completed}/${allSets.length}</b> sets complete</p></section>
    ${rest || workout.restTimer?.paused ? `<section class="rest-timer" aria-live="off"><div><small>${workout.restTimer?.paused ? "REST PAUSED" : "RECOVER & RESET"}</small><strong data-rest-display ${restAttrs}>${formatRest(rest || workout.restTimer?.durationSeconds || 0)}</strong><span>${paused ? "Resume your workout to continue." : "Take a breath. Your next set is waiting."}</span></div><div><button data-action="adjust-rest" ${restAttrs} data-value="-15" ${paused ? "disabled" : ""} aria-label="Shorten rest by 15 seconds">−15</button><button data-action="stop-rest" ${restAttrs} ${paused ? "disabled" : ""}>Skip</button><button data-action="adjust-rest" ${restAttrs} data-value="15" ${paused ? "disabled" : ""} aria-label="Extend rest by 15 seconds">+15</button></div></section>` : ""}
    ${renderStrengthSetupHelper(exercise, current, workout.units || state.settings.units)}
    <section class="set-logger card"><header><span><b>Sets</b><small>${paused ? "Resume before editing sets" : "Tap the circle after the set"}</small></span><button class="text-button" data-action="add-set" ${identityAttrs} ${paused ? "disabled" : ""}>+ Add set</button></header><div class="set-grid set-grid-head"><span>SET</span><span>${escapeHtml((workout.units || state.settings.units).toUpperCase())}</span><span>REPS</span><span>RPE</span><span>DONE</span></div>${current.sets.map((set,setIndex) => `<div class="set-grid ${set.done ? "done" : ""} ${set.error ? "invalid" : ""}"><span class="set-index">${set.kind === "warmup" ? "W" : setIndex + 1}</span><label><span class="sr-only">Weight set ${setIndex+1}</span><input inputmode="decimal" type="number" min="0" max="5000" step="0.5" data-action="set-field" data-field="weight" data-workout-id="${escapeHtml(workout.id)}" data-exercise-id="${escapeHtml(current.exerciseId)}" data-set-id="${escapeHtml(set.id)}" value="${set.weight}" ${paused ? "disabled" : ""}></label><label><span class="sr-only">Reps set ${setIndex+1}</span><input inputmode="numeric" type="number" min="0" max="1000" data-action="set-field" data-field="reps" data-workout-id="${escapeHtml(workout.id)}" data-exercise-id="${escapeHtml(current.exerciseId)}" data-set-id="${escapeHtml(set.id)}" value="${set.reps}" aria-invalid="${set.error ? "true" : "false"}" ${paused ? "disabled" : ""}></label><label><span class="sr-only">RPE set ${setIndex+1}</span><input inputmode="decimal" type="number" min="1" max="10" step="0.5" data-action="set-field" data-field="rpe" data-workout-id="${escapeHtml(workout.id)}" data-exercise-id="${escapeHtml(current.exerciseId)}" data-set-id="${escapeHtml(set.id)}" value="${set.rpe ?? ""}" placeholder="—" ${paused ? "disabled" : ""}></label><button class="set-check" data-action="toggle-set" data-workout-id="${escapeHtml(workout.id)}" data-exercise-id="${escapeHtml(current.exerciseId)}" data-set-id="${escapeHtml(set.id)}" aria-label="${set.done ? "Mark incomplete" : "Complete"} set ${setIndex+1}" aria-pressed="${set.done}" ${paused ? "disabled" : ""}>${set.done ? icon("check") : ""}</button>${set.error ? `<p class="set-error" role="alert">${escapeHtml(set.error)}</p>` : ""}</div>`).join("")}</section>
    <details class="training-instruction training-workout-notes"><summary><span>${icon("plus")} Workout notes</span>${icon("chevron")}</summary><div><label for="workout-notes" class="sr-only">Workout notes</label><textarea id="workout-notes" data-workout-id="${escapeHtml(workout.id)}" maxlength="2000" placeholder="How did the session feel?">${escapeHtml(workout.notes || "")}</textarea><p class="field-error" data-workout-notes-error role="alert" hidden></p></div></details>
    <details class="training-instruction"><summary><span>${icon("equipment")} Session controls</span>${icon("chevron")}</summary><section class="active-tools"><button data-action="swap-active-exercise" ${identityAttrs} ${paused ? "disabled" : ""}>${icon("swap")}<span><b>Swap exercise</b><small>Before any set is logged</small></span></button><button data-action="reorder-active-exercise" ${identityAttrs} data-neighbor-set-id="${escapeHtml(previous?.sets[0]?.id || "")}" data-direction="-1" ${currentIndex === 0 ? "disabled" : ""}>${icon("grip")}<span><b>Move up</b><small>Earlier in the session</small></span></button><button data-action="reorder-active-exercise" ${identityAttrs} data-neighbor-set-id="${escapeHtml(next?.sets[0]?.id || "")}" data-direction="1" ${currentIndex >= workout.exercises.length - 1 ? "disabled" : ""}>${icon("grip")}<span><b>Move down</b><small>Later in the session</small></span></button><button data-action="toggle-workout-pause" data-workout-id="${escapeHtml(workout.id)}" data-pause-key="${escapeHtml(workoutPauseKey(workout))}">${icon(workout.status === "paused" ? "play" : "pause")}<span><b>${workout.status === "paused" ? "Resume" : "Pause"}</b><small>Your sets stay saved</small></span></button></section></details>
    <section class="exercise-navigation card"><button data-action="previous-exercise" ${identityAttrs} data-neighbor-set-id="${escapeHtml(previous?.sets[0]?.id || "")}" ${previous ? "" : "disabled"}>${icon("chevron")} Previous</button><button data-action="next-exercise" ${identityAttrs} data-neighbor-set-id="${escapeHtml(next?.sets[0]?.id || "")}" ${next ? "" : "disabled"}>Next ${icon("chevron")}</button></section>
    ${next ? `<section class="next-exercise card"><span class="eyebrow">UP NEXT</span>${exercisePoster(exerciseById(next.exerciseId) || next,{className:"thumb"})}<div><b>${escapeHtml(next.snapshot.name)}</b><small>${next.target.sets} × ${next.target.reps}</small></div><button class="icon-only" data-action="next-exercise" ${identityAttrs} data-neighbor-set-id="${escapeHtml(next?.sets[0]?.id || "")}" aria-label="Go to next exercise">${icon("chevron")}</button></section>` : ""}
    <div class="workout-finish-bar">${button({label:"Finish workout",action:"finish-workout",variant:"primary",extra:`data-workout-id="${escapeHtml(workout.id)}"`})}</div>
  </div>`;
}
