...
function workoutPlan({ state, plan, exerciseById }) {
  ...
  return `<div class="train-workout-view">
    ...
    <section class="training-principles card"><header class="section-heading"><div><span class="eyebrow">WHY THIS SESSION</span><h2>A plan with a reason</h2></div><span class="soft-badge">Built for you</span></header><div class="principle-rail"><article><span class="principle-icon">${icon("spark")}</span><b>01</b><strong>Goal matched</strong><small>Movement patterns support ${escapeHtml(titleCase(state.profile.goal))}.</small></article><article><span class="principle-icon">${icon("equipment")}</span><b>02</b><strong>Equipment ready</strong><small>Every move fits today’s ${escapeHtml(titleCase(plan.location))} setup.</small></article><article><span class="principle-icon">${icon("progress")}</span><b>03</b><strong>History aware</strong><small>Past loads appear only when a comparable log exists.</small></article></div><div class="principle-rail-cue" aria-hidden="true"><span>Swipe for more</span><i></i><i></i><i></i></div></section>
  </div>`;
}
...
function exerciseDetail({ state, exercise, motionPaused = false, replacing = false }) {
  ...
  return `<article class="exercise-detail">
    ...
    <section class="exercise-detail-visual card ${motion ? "motion-guide" : "static-guide"}">
      <header class="detail-media-heading"><div><span class="eyebrow">TECHNIQUE GUIDE</span><h2>${escapeHtml(exercise.name)}</h2></div><span class="guide-status-chip">${motion ? "Technique video" : visualGuide ? "Technique visual" : "Written guide"}</span></header>
